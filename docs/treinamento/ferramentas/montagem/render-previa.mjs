...
const dir = process.argv[3]; const saida = process.argv[4] || path.join(dir, "quadros");
...
for (let i = 0; i < n; i++) {
  ...
  await p.screenshot({ path: path.join(saida, `${String(i).padStart(4, "0")}.jpg`), type: "jpeg", quality: 92 });
}
await b.close();
